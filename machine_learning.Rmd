...
A hard to swallow assumption in linear models is that changes in a particular feature will increase at the same rate across all values on a variable. General Additive Models (GAM) give analysts the opportunity to identify nonlinear changes between covariates and the dependent variable. These nonparametric models can suffer from poor interpretability, however, and strong assumptions will have to be made regarding the data generation process. In a project using census data in the city of Los Angeles, gam models help us to identify outlier trends in homeless individuals, yet also reveal limitations of the loess smoother at upper boundaries of data.

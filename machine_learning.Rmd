...
A hard to swallow assumption in linear models is that, at all values an instance has in a particular feature, increasing the value by one unit will have the same effect on the predicted outcome for each instance. General Additive Models are a nonparametric way to make use of feature categorization (kind of like stepwise regression) by putting variables into separate features to assess fitting, and giving the analyst an opportunity to analyze fluid changes in variable features. Unfortunately, gam models can suffer from poor interpretability, and strong assumptions will have to be made regarding the data generation process. In a project using data from the census on homeless counts in the city of Los Angeles, we see one of these issues clearly: the loess smoother perform poorly at the upper boundaries of data.
